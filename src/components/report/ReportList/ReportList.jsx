...
import { getReports, maskPost } from '../../../api/reports';
...
// 커스텀 입력 컴포넌트
const CustomInput = forwardRef(({ value, onClick, onChange, inputText, setInputText, onDateSelect }, ref) => {
...
const ReportList = () => {
  ...
  const handleMaskPost = async (report) => {
    ...
    if (response.success) {
      ...
      // 실제 구현 시에는 리스트를 새로고침하거나 상태를 업데이트해야 함
      fetchReports(); 
    } else {
      alert(response.result);
    }
        ...
        {loading ? (
          ...
          reports.map((report) => (
            <ReportItem 
              ...
              content="상품 상세 보기 및 숨김 처리가 가능한 게시물입니다."
